...
export type Premium =
  | { plan: 'vie'; since: number }
  | { plan: 'an' | 'mois'; since: number; until: number; renew: boolean; trialEnd?: number; promo?: boolean };

/** Offre choisie : les 3 du paywall et l'offre de sortie à 39,99 $ la 1re année. */
export type OffreId = 'an' | 'mois' | 'vie' | 'an39';
...
export const enEssai = (p: Premium | null, now: number = Date.now()) => !!p && p.plan !== 'vie' && !!p.trialEnd && now < p.trialEnd;
...
/** Abonnement après l'achat (buySheet) : à vie, annuel avec 3 jours d'essai (39,99 $ pour l'offre de sortie), ou mensuel. */
export function nouvelAbonnement(id: OffreId, now: number = Date.now()): Premium {
  if (id === 'vie') return { plan: 'vie', since: now };
...
export function ligneAbonnement(p: Premium | null, now: number = Date.now()): string {
  ...
  if (p.plan === 'vie') return 'Accès à vie';
