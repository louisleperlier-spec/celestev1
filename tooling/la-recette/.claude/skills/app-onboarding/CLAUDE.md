...
A Claude Code skill (`app-onboarding`) that designs and builds high-converting questionnaire-style onboarding flows for any app. Modelled on proven conversion patterns from top subscription apps (Mob, Headspace, Noom, Duolingo). Invoked via `/app-onboarding` from within a user's app project.
...
The skill implements a 13-screen archetype sequence based on a best-in-class recipe app's onboarding (the gold-standard reference). The psychological sequence is:
